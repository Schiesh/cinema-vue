    ...
    <table v-else class="table">
      ...
      <tbody>
        <tr v-for="movie in movies" :key="movie.id">
          ...
          <td><span class="badge">{{ movie.rating }}</span></td>
          <td class="green">${{ parseFloat(movie.price).toFixed(2) }}</td>
          <td>
            <button class="btn btn-secondary btn-sm" @click="openModal(movie)">Edit</button>
            <button class="btn btn-danger btn-sm" @click="removeMovie(movie.id)">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  data() {
    return {
      ...
      form: { title: "", genre: "", duration: "", rating: "PG-13", price: "" }
    }
  },
  ...
  methods: {
    ...
    async saveMovie() {
      if (!this.form.title || !this.form.price) {
        this.$emit("message", { text: "Title and price are required.", type: "error" })
        return
      }
      try {
...
.btn-sm { padding: 0.4rem 0.8rem; font-size: 0.8rem; margin-right: 4px; }
</style>
