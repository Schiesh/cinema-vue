<template>
  <div>
    ...
    <div v-else-if="allShowings.length === 0" class="empty">
    ...
    <table v-else class="table">
      ...
      <tbody>
        <tr v-for="showing in allShowings" :key="showing.id">
          ...
          <td class="muted">
            {{ showing.screen_name || `Screen ${showing.screen_number}` }}
          </td>
          <td>{{ formatShowtime(showing.showtime) }}</td>
          <td class="green">
            ${{ parseFloat(showing.price || 0).toFixed(2) }}
          </td>
          <td class="muted">
            {{ showing.seats_available }}/{{ showing.seats_total }}
          </td>
          <td>
            <div class="progress-wrap">
              <div
                class="progress-bar"
                :style="{ width: `${showing.percent_full}%` }"
              ></div>
            </div>
            <span class="muted" style="font-size: 0.8rem"
              >{{ showing.percent_full }}%</span
            >
          </td>
          <td>
            <button
              class="btn btn-danger btn-sm"
              @click="cancelShowing(showing.id)"
            >
              Cancel
            </button>
          </td>
  ...
  methods: {
    async loadShowings() {
      ...
      try {
        ...
        this.allShowings = showings.sort(
    ...
    async saveShowing() {
      ...
      const screen = this.screens.find(
        (s) => s.id === parseInt(this.form.screen_id),
      );
